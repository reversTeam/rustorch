//! T246.10 TrackE.3 / TrackE.4 — Parity tests for the sort-permutation
//! Group-GEMM kernels :
//!   - `mul_mm_id_gemm_q4_k_sorted_bf16` vs `mul_mm_id_gemm_q4_k_bf16` (TrackE.3)
//!   - `mul_mm_id_gemm_q5_k_sorted_bf16` vs `mul_mm_id_gemm_q5_k_bf16` (TrackE.4)
//!
//! All four kernels :
//!   - Read the same `[M, K]` BF16 activations
//!   - Read the same `[M, k_used]` int32 topk_indices
//!   - Write the same `[M, k_used, N]` BF16 output layout
//!   - Use byte-for-byte identical FP arithmetic (warp-shuffle Q4_K / Q5_K
//!     BF16 dot)
//!
//! Only the schedule changes : the sorted kernels iterate a compact
...
// ─── TrackE.4 — Q5_K sort-permutation parity ─────────────────────────────

#[allow(clippy::too_many_arguments)]
fn run_sorted_parity_q5k(label: &str, m_tokens: usize, n_rows: usize, k_dim: usize, k_used: usize) {
    let n_experts = 128usize;
    ...
    // Build Q5_K expert weight blobs and upload.
    let blobs: Vec<Vec<u8>> = (0..n_experts)
    ...
    // Build permutation on device — same helper as Q4_K.
    unsafe {
        let (tp, _g) = topk_dev.device_ptr(&stream);
        let (ip1, _g1) = ids_src1.device_ptr(&stream);
        let (ip2, _g2) = ids_dst.device_ptr(&stream);
    ...
    // Reference output via the unsorted TrackE.2 Q5_K kernel.
    let mut y_ref = stream
        .alloc_zeros::<half::bf16>(m_tokens * k_used * n_rows)
        .expect("alloc y_ref");
    unsafe {
        ...
        kernels
            ...
            .expect("mul_mm_id_gemm_q5_k_bf16");
    }

    // Sorted output via TrackE.4 Q5_K kernel.
    let mut y_sorted = stream
        .alloc_zeros::<half::bf16>(m_tokens * k_used * n_rows)
        .expect("alloc y_sorted");
    unsafe {
        ...
        kernels
            ...
            .expect("mul_mm_id_gemm_q5_k_sorted_bf16");
    }
...
#[test]
#[ignore = "requires CUDA GPU — run with --ignored on DGX"]
fn sorted_q5k_parity_small() {
    run_sorted_parity_q5k("sorted-q5k M=8 N=128 K=256", 8, 128, 256, 8);
}

#[test]
#[ignore = "requires CUDA GPU — run with --ignored on DGX"]
fn sorted_q5k_parity_m32() {
    run_sorted_parity_q5k("sorted-q5k M=32 N=128 K=256", 32, 128, 256, 8);
}

#[test]
#[ignore = "requires CUDA GPU — run with --ignored on DGX"]
fn sorted_q5k_parity_medium() {
    run_sorted_parity_q5k("sorted-q5k M=128 N=1024 K=2048", 128, 1024, 2048, 8);
}

#[test]
#[ignore = "requires CUDA GPU — run with --ignored on DGX"]
fn sorted_q5k_parity_large() {
    // Approximate Qwen3.6-35B-A3B MoE FFN shape : 128 experts, ef=N=4096
    // (instead of 18944 to keep test cost reasonable), K=d=2048.
    run_sorted_parity_q5k("sorted-q5k M=512 N=4096 K=2048", 512, 4096, 2048, 8);
}
